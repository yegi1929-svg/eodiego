...
function Login({ onClose, onLogin, onSignup }: { onClose: () => void; onLogin: (e: FormEvent) => void; onSignup: () => void }) { return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="login-title"><form className="modal-card" onSubmit={onLogin}><button type="button" className="close-button" onClick={onClose}>×</button><div className="brand-mark">GO</div><p className="eyebrow">WELCOME BACK</p><h2 id="login-title">다시 만나네요, 여행자님</h2><p className="modal-copy">로그인하고 저장한 일정과 제주 추천을 확인하세요.</p><label>아이디<input required placeholder="아이디를 입력하세요" /></label><label>비밀번호<input required type="password" placeholder="비밀번호를 입력하세요" /></label><button className="primary-button full">로그인하기</button><button type="button" className="signup-link" onClick={onSignup}>아직 어디GO 회원이 아니신가요? <b>회원가입</b></button><button type="button" className="text-button" onClick={onClose}>계속 둘러보기</button></form></div>; }
