...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    <section className={tab === "planner" ? "welcome-band planner-hero" : "welcome-band"}>
      <div className={tab === "planner" ? "hero-copy" : undefined}>{tab === "planner" ? <><p className="eyebrow">JEJU TRIP EDITION</p><h1>오늘 제주,<br /><strong>어디GO?</strong></h1><p>취향에 맞는 동선부터 지금 한적한 장소까지<br />여행의 망설임을 가볍게 덜어드릴게요.</p><span className="hero-sticker">슝! 코스 생성</span></> : <><p className="eyebrow">JEJU TRIP</p><h1>오늘 제주, <strong>어디GO?</strong></h1><p>취향에 맞는 동선부터 지금 한적한 장소까지<br />여행의 망설임을 가볍게 덜어드릴게요.</p></>}</div>
...
function Login({ onClose, onLogin, onSignup }: { onClose: () => void; onLogin: (e: FormEvent) => void; onSignup: () => void }) { return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="login-title"><form className="modal-card" onSubmit={onLogin}><button type="button" className="close-button" onClick={onClose}>×</button><div className="brand-mark">GO</div><p className="eyebrow">WELCOME BACK</p><h2 id="login-title">다시 만나요, 여행자님</h2><p className="modal-copy">로그인하고 저장한 일정과 실시간 추천을 확인하세요.</p><label>아이디<input required placeholder="아이디를 입력하세요" /></label><label>비밀번호<input required type="password" placeholder="비밀번호를 입력하세요" /></label><button className="primary-button full">로그인하기</button><button type="button" className="signup-link" onClick={onSignup}>아직 어디GO 회원이 아니신가요? <b>회원가입</b></button><button type="button" className="text-button" onClick={onClose}>계속 둘러보기</button></form></div>; }
...
function Signup({ onBack, onSubmit }: { onBack: () => void; onSubmit: (e: FormEvent) => void }) { return <main className="signup-shell"><section className="signup-panel"><button className="text-button back" onClick={onBack}>← 돌아가기</button><div className="brand-mark">GO</div><p className="eyebrow">JEJU TRIP COMPANION</p><h1>여행 취향을 알려주세요</h1><p className="signup-copy">가입 후에는 내 일정 저장과 실시간 맞춤 추천을 이용할 수 있어요.</p><form className="signup-form" onSubmit={onSubmit}><label>이메일<input required type="email" placeholder="you@example.com" /></label><label>닉네임<input required placeholder="여행에서 부를 이름" /></label><div className="two-inputs"><label>이름<input required placeholder="이름" /></label><label>전화번호<input required type="tel" placeholder="010-0000-0000" /></label></div><label>연락처<input placeholder="선택 · 인스타그램, 카카오톡 등" /></label><label className="check-row"><input type="checkbox" required /> 필수 약관과 개인정보 처리방침에 동의합니다.</label><button className="primary-button full">가입 완료하고 시작하기</button></form></section></main>; }
